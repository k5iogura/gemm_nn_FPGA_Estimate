...
void contextNQ(cl_device_id device_Id, cl_context *conteXt, int N, cl_command_queue queUe[]){
    cl_int ret;
    int i;
    /* Create OpenCL context */
    *conteXt = clCreateContext (NULL, 1, &device_Id, NULL, NULL, &ret);
    ...
    for(i=0;i<N;i++){
        /* Create Command Queue */
        queUe[i] = clCreateCommandQueue (*conteXt, device_Id, 0, &ret);
        checkErr(ret,"clCreateCommandQueue");
    }
}
...
void find_CnKQ(
    ...
    cl_program program=NULL;
    if(device_id)
        contextNQ(device_id, context, n, queue);
    if(*context)
        program = cProgram(kernel_file,*context,device_id);
    if(program)
        for(i=0;i<n;i++)
