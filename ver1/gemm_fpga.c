...
//static errors(char *msg){perror(msg);exit(-1);}
extern void gemm_nn_cpu(int M, int N, int K, float ALPHA, 
        float *A, int lda, 
        float *B, int ldb,
        float *C, int ldc);
...
#else
  fprintf(stderr,"load from binary onARM\n");
  program =
...
/* Finalization */
  ...
  fprintf(stderr,"gemm_fpga_finalized.\n");
  return ;
}
...
void gemm_nn(int M, int N, int K, float ALPHA, 
        ...
        float *C, int ldc) {
#if 0
  gemm_nn_cpu (M,N,K,ALPHA,A,lda,B,ldb,C,ldc);
#endif
